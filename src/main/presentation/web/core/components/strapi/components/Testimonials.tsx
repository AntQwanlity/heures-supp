...
type Props = {
  title: string;
  subtitle: string;
  testimonials: { content: string; author: { name: string; title: string; avatar: StrapiImage } }[];
};

export const Testimonials: React.FC<Props> = ({ title, subtitle, testimonials }) => {
  return (
              ...
              <figure className="relative rounded-2xl bg-white p-6 shadow-xl shadow-slate-900/10">
                ...
                <figcaption className="relative mt-6 flex items-center justify-between border-t border-slate-100 pt-6">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-500">{testimonial.author.title}</div>
                  </div>
