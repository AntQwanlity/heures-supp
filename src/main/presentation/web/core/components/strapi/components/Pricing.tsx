...
type SwirlyDoodleProps = {
  className?: string;
};

const SwirlyDoodle: React.FC<SwirlyDoodleProps> = ({ className }) => {
  return (
  ...
  return (
    <section
      className={clsx(
        ...
        highlighted ? "bg-blue-600 py-8" : "lg:py-8",
      )}
    >
      <h3 className="mt-5 font-display text-lg text-white">{name}</h3>
      <p className={clsx("mt-2 text-base", highlighted ? "text-white" : "text-slate-400")}>
        {description}
      </p>
      ...
      >
        {features.map((feature) => (
          <li key={feature} className="flex">
            <CheckIcon className={highlighted ? "text-white" : "text-slate-400"} />
...
export const Pricing: React.FC<Props> = ({ title, subtitle, pricingPlans }) => {
  const splittedTitle = title.split(/(.*)\[(.+)\](.*)/).filter(Boolean);
        ...
        <div className="md:text-center">
          <h2 className="font-display text-3xl tracking-tight text-white sm:text-4xl">
            ...
            <span className="relative whitespace-nowrap">
              ...
              <span className="relative">{splittedTitle[1]}</span>
            </span>{" "}
            {splittedTitle[2]}
          </h2>
        ...
        <div className="-mx-4 mt-16 grid max-w-2xl grid-cols-1 gap-y-10 sm:mx-auto lg:-mx-8 lg:max-w-none lg:grid-cols-3 xl:mx-0 xl:gap-x-8">
          {pricingPlans.map((plan) => (
            <Plan
              key={`plan-${plan.name}`}
              name={plan.name}
              price={Money.fromUnit(plan.price).format()}
